...
# Matplotlib
import matplotlib
matplotlib.use('Agg')  # Non-interactive for parallel envs
...
class PatchFunnelEnvV1(gym.Env):
    ...
    def _visualize(self):
        ...
        self._ax.set_title(
            ...
            f'Patch: v={self.patch.v:.1f}, θ={np.degrees(self.patch.theta):.0f}° | '
        ...
        ellipse = Ellipse(
            ...
            facecolor='cyan' if agents_inside == self.num_agents else 'yellow',
            edgecolor='darkblue',
            alpha=0.3,
            linewidth=3
        )
        ...
        for i in range(self.num_agents):
            ...
            self._ax.plot(x, y, marker, color=color, markersize=12,
        ...
        margin = max(self.patch.a, self.patch.b) + 3
...
def train_patch_funnel_v1(
    ...
    callback = FunnelCallbackV1(run_dir, checkpoint_freq)
...
def run_eval_v1(model_path, num_episodes=5):
    ...
    print("\nV1 Features:")
    print("  - Patch has CAR-LIKE dynamics")
    print("  - Patch theta comes from heading (state)")
    print("  - Agents use neuro-controller")
    print("=" * 70)
