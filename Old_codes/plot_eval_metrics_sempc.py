...
from drl_patch_funnel import PatchFunnelEnv
...
def evaluate_and_plot(model_path, num_episodes=10, output_dir=None):
    ...
    env = PatchFunnelEnv(
        num_agents=4,  # FIX: Match training!
        render_mode=None,
    )
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            cumulative_reward += reward[0]  # FIX: Uncommented!
            step += 1  # FIX: Uncommented!
            
            # Calculate patch velocity magnitude 
            patch_velocity = np.sqrt(inner_env.patch.vx**2 + inner_env.patch.vy**2)
            
            metrics['timestep'].append(step)
            metrics['patch_velocity'].append(patch_velocity)
            metrics['patch_x'].append(inner_env.patch.cx)
            metrics['patch_y'].append(inner_env.patch.cy)
            metrics['lap_progress'].append(info[0].get('lap_progress', 0) * 100)
            metrics['cumulative_reward'].append(cumulative_reward)

            # Calculate containment rate
            agents_inside = 0
            for i in range(inner_env.num_agents):
                x = inner_env.current_obs["poses_x"][i]
                y = inner_env.current_obs["poses_y"][i]
                if inner_env.patch.is_inside(x, y):
                    agents_inside += 1
            metrics['containment_rate'].append(agents_inside / inner_env.num_agents)

            # Calculate average agent distance from patch center
            dists = []
            for i in range(inner_env.num_agents):
                x = inner_env.current_obs["poses_x"][i]
                y = inner_env.current_obs["poses_y"][i]
                dist = np.sqrt((x - inner_env.patch.cx)**2 + (y - inner_env.patch.cy)**2)
                dists.append(dist)
            metrics['agent_dist'].append(np.mean(dists))

            if done[0]:
                final_progress = metrics['lap_progress'][-1]
                print(f"  Ep {ep+1}: steps={step}, reward={cumulative_reward:.0f}, progress={final_progress:.1f}%")
                break
        
        all_episodes.append(metrics)
    
    vec_env.close()  # FIX: Added!
