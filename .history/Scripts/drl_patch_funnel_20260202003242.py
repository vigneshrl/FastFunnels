...
class DeformableEllipsoid:
    ...
    #Patch knows world coordinates and does obstacle avoidance 
    def update_position(self, dt, obstacles_lidar=None, safety_margin=1.0):
        """
        Update patch position with velocity and obstacle avoidance.
        
        Args:
            dt: Time step
            obstacles_lidar: LIDAR scan data for obstacle detection
            safety_margin: Minimum distance to obstacles
        """
        # Proposed new position
        new_cx = self.cx + self.vx * dt
        new_cy = self.cy + self.vy * dt
        
        # Obstacle avoidance using LIDAR (if available)
        if obstacles_lidar is not None:
            # Check if proposed position is safe
            # Use repulsive potential field from walls
            repel_x, repel_y = 0.0, 0.0
            
            n_rays = len(obstacles_lidar)
            for i, dist in enumerate(obstacles_lidar):
                if dist < safety_margin * 2:  # Only consider close obstacles
                    # Angle of this LIDAR ray
                    angle = -np.pi/2 + i * np.pi / n_rays  # Adjust based on your LIDAR FOV
                    
                    # Repulsive force away from obstacle
                    strength = (safety_margin * 2 - dist) / (safety_margin * 2)
                    repel_x -= strength * np.cos(angle) * 2.0
                    repel_y -= strength * np.sin(angle) * 2.0
...
class SEMPCSolver:
    ...
    def __init__(self, robot_radius=0.15, wheelbase=0.33, num_neighbors=3):
        ...
        # MPC parameters
        self.T_horizon = 1.0  # Prediction horizon (seconds) # Reduced from 2.0 to 1.0 to make MPC faster
        self.N = 5  # Number of steps # Reduced from 10 to 5 to make MPC faster
    ...
    def solve(self, x0, patch, neighbor_positions):
        ...
        Args:
            ...
            neighbor_positions: List of [x, y] for neighbors - now in local coordinates of the patch
        ...
        try:
            ...
            for k in range(self.N + 1):
                t = k * self.dt
                # This should be in local coordinates of the patch
                cx_traj[k] = patch.vx * t
                cy_traj[k] = patch.vy * t
...
class PatchFunnelEnv(gym.Env):
    ...
    def __init__(self, num_agents=4, render_mode="human"):
        ...
        self.cached_controls = [None] * num_agents
        self.cached_feasible = [False] * num_agents
    ...
    #OLD CODE FOR GOAL BASED REWARD FUNCTION
        ...
        if safety_rate > 0.3:
            reward -= 0.5 * (safety_rate - 0.3)  # Penalty for high intervention
        

        # ===== 8.PATCH LEAVING AGENTS PENALTY =====
        dist_patch_to_centroid = np.sqrt(
            (self.patch.cx - centroid[0])**2 + 
            (self.patch.cy - centroid[1])**2
        )

        # Penalty if patch is far from agents
        if dist_patch_to_centroid > 3.0:
            reward -= 2.0 * (dist_patch_to_centroid - 3.0)

        # Extra penalty if agents are outside AND patch is far
        if containment_rate < 1.0 and dist_patch_to_centroid > 2.0:
            reward -= 3.0
    ...
    def reset(self, seed=None, options=None):
        ...
        self.initial_patch_pos = np.array([centroid[0], centroid[1]])
        # Reset tracking
        self.step_count = 0
        self.mpc_successes = 0
        self.mpc_attempts = 0
    ...
    def step(self, action):
        ...
        # Get LIDAR data for obstacle avoidance (use lead agent's LIDAR)
        lidar_scan = self.current_obs["scans"][0]
        # Update patch position with obstacle avoidance (NOT teleport to centroid)
        self.patch.update_position(dt=0.1, obstacles_lidar=lidar_scan, safety_margin=1.5)
        self.patch.save_state()
        solve_mpc = (self.step_count == 1) or(self.step_count % 5 == 0) # Solve MPC every 5 steps to make it faster


        # ===== 2. SE-MPC SOLVES FOR EACH AGENT =====
        # Convert agent positions to local coordinates of the patch
        positions_world = [[self.current_obs["poses_x"][i], self.current_obs["poses_y"][i]] 
                     for i in range(self.num_agents)]
        
        #patch center in world coordinates 
        patch_cx_world = self.patch.cx 
        patch_cy_world = self.patch.cy
        #convert to local(Patch-centered) frame
        positions_local = [[p[0] - patch_cx_world, p[1] - patch_cy_world] for p in positions_world]
        ...
        for i in range(self.num_agents):
            # Agent state
            x_local = self.current_obs["poses_x"][i] - patch_cx_world
            y_local = self.current_obs["poses_y"][i] - patch_cy_world
            theta_i = self.current_obs["poses_theta"][i]
            v = self.prev_v[i]
            
            x0_local = np.array([x_local, y_local, theta_i, v])
            ...
            # self.mpc_attempts += 1
            # u_opt, feasible = self.mpc_solvers[i].solve(x0_local, self.patch, neighbors_local)
        ...
        # Goal reached
        progress, _, _ = self._get_lap_progress(np.array([self.patch.cx, self.patch.cy]))
        ...
        if self.step_count % 5 == 0:
            self._visualize(positions_local, mpc_feasible)
        ...
        return obs, reward, terminated, truncated, {
            ...
            "safety_intervention_rate": self.safety_interventions / max(1, self.step_count * self.num_agents)
        }
    ...
    class FunnelCallback(BaseCallback):
        def __init__(self, save_dir, save_freq, verbose=1):
            ...
            self.best_reward = -np.inf
                self.start_time = time.time()  # ADD THIS
                self.last_log_time = time.time()  # ADD THIS
                self.last_log_timesteps = 0 # ADD THIS
            ...
            if self.locals.get("dones", [False])[0]:
                ...
                if self.episode_count % 10 == 0:
                    ...
                    if avg_r > self.best_reward:
                        ...
                        recent_reasons = self.episode_rewards[-50:]
                        reason_counts = {}
