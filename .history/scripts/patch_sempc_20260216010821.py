...
class PatchEnv(gym.Env):
    ...
    def _compute_reward(self, base_obs, lidar_info):
        ...
        if self.patch.v > 0.1:  # Only reward if moving
            ...
            reward += 7.0 * v_normalized  # Strong reward for speed (was 5.0)
        ...
        if min_dist < 1.5:
            reward -= 8.0 * (1.5 - min_dist)  # Large penalty for dangerous proximity
    ...
    def step(self, patch_action):
        ...
        for i in range(self.num_agents):
            ...
            # Integrate agent speed (env expects [speed, steering_angle])
            v_new = v_i + accel * dt
