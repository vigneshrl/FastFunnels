...
import numpy as np
import matplotlib
# Use TkAgg backend to avoid conflicts with pygame
...
from NeuroPatch import PatchEnv
...
def evaluate_patch_policy(
    ...
    vecnorm_path = patch_policy_path.replace(".zip", "").replace("best_model", "best_vecnormalize.pkl")
    if not vecnorm_path.endswith(".pkl"):
        vecnorm_path = patch_policy_path + "_vecnormalize.pkl"
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            if render and hasattr(inner_env,'_visualize'):
...
def evaluate_agent_policy(
    ...
    # Load normalization stats
    vecnorm_path = agent_policy_path.replace(".zip", "").replace("best_model", "best_vecnormalize.pkl")
    if not vecnorm_path.endswith(".pkl"):
        vecnorm_path = agent_policy_path + "_vecnormalize.pkl"
