...
class PatchEnv(gym.Env):
    ...
    def __init__(self, num_agents=2, render_mode=None, domain_randomize=True, 
        ...
        # Action bounds
        self.patch_a_range = (1.5, 5.0)
        self.patch_b_range = (1.5, 5.0)
        ...
        # Agent relative positions: num_agents × 2
        # Agent relative velocities: num_agents × 2
        # Agents inside count (1)
        # Domain randomization params (1)
        # Alpha (1)
        obs_dim = 4 + 2 + 16 + 2 + 3 + 1 + 1 + 16 # Added 16 for clearance
    ...
    # Get a point 1 unit away in goal direction (in world frame)
        ...
        obs = np.concatenate([
            ...
            heading_obs,    # 2
            goal_obs,       # 3
            clearance_obs,  #(NEW: Clearance = LIDAR - ellipse radius)
            # agent_pos_obs,  # num_agents * 2
            # agent_vel_obs,  # num_agents * 2
            # inside_obs,     # 1
            domain_obs,     # 1
            alpha_obs       # 1
        ]).astype(np.float32)
    ...
    def _compute_reward(self, base_obs, lidar_info):
        ...
        if self.patch.v > 0.1:  # Only reward if moving
            # Normalized velocity reward: peak at 2.5 m/s
            v_normalized = min(self.patch.v / 2.5, 1.0)  # Normalize to [0, 1]
            reward += 2.0 * v_normalized  # Dense reward for any forward movement
            
            # Bonus for maintaining good speed
            if 1.5 <= self.patch.v <= 3.5:
                reward += 1.0  # Sweet spot bonus
        else:
        ...
        if min_dist > 0.5:  # Only reward if we have some clearance
            # Normalize clearance: optimal around 2.0-3.0m
            clearance_reward = min(min_dist / 3.0, 1.0)  # Normalize to [0, 1]
            reward += 1.5 * clearance_reward  # Dense reward for safety margin
        else:
            # Penalty for being too close
            reward -= 5.0 * (0.5 - min_dist)  # Exponential penalty as we get closer
        
        # Additional penalty for very close calls
        if min_dist < 1.0:
            reward -= 20.0 * (1.0 - min_dist)  # Large penalty for dangerous proximity
        ...
        if progress_delta > 0:
            ...
            reward -= 10.0 * abs(progress_delta)  # Penalty for going backward
        ...
        # Normalize patch sizes using safe minimums
        size_ratio_a = (self.patch.a - safe_min_a) / (max_a - safe_min_a)
        size_ratio_b = (self.patch.b - safe_min_b) / (max_b - safe_min_b)
        ...
        # Optimal size ratio is around 0.3-0.5 (moderate size)
        optimal_ratio = 0.4
        ...
        if forward_clearance < 4.0:  # Tight forward/back
            ...
            reward += 2.0 * shrink_bonus_a * (4.0 - forward_clearance) / 4.0  # More reward when tighter
        
        if left_right_clearance < 4.0:  # Tight left/right
            shrink_bonus_b = (1.0 - size_ratio_b)  # Reward for smaller b
            reward += 2.0 * shrink_bonus_b * (4.0 - left_right_clearance) / 4.0  # More reward when tighter
        
        # Penalty for excessive size
        if self.patch.a > 3.0:
            reward -= 0.5 * (self.patch.a - 3.0)
        if self.patch.b > 2.5:
            reward -= 0.5 * (self.patch.b - 2.5)
