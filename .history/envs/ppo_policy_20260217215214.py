...
class PatchEnvConfig:
    ...
    frenet_edge_penalty_weight: float = 25.0
    trackwidth_violation_penalty: float = 500.0
    patch_wall_collision_penalty: float = 250.0
    edge_guard_enabled: bool = True
    edge_guard_start_ratio: float = 0.55
    edge_guard_full_ratio: float = 0.90
    edge_guard_min_accel_scale: float = 0.35
    edge_guard_min_steer_scale: float = 0.70
    edge_guard_centering_steer_gain: float = 0.12
    ...
    # old: agent_speed_boost: float = 1.15
...
class PatchEnv(gym.Env):
    ...
    def _pack_base_action(self, steering_cmd: float, speed_cmd: float) -> np.ndarray:
        ...
        for idx, name in enumerate(control_names):
            ...
            if key == "speed":
                ...
                # Conservative fallback keeps prior behavior by position.
                action[idx] = float(steering_cmd if idx == 0 else speed_cmd)
        return action
    ...
    def _clip_base_actions(self, actions: np.ndarray) -> np.ndarray:
        ...
        for idx, name in enumerate(control_names):
            key = str(name).lower()
            if key == "speed":
                # old: speed clip used self.cfg.stable_agent_speed_cap directly.
                speed_cap = min(float(self.cfg.stable_agent_speed_cap), float(self.cfg.hard_speed_cap))
                clipped[:, idx] = np.clip(clipped[:, idx], 0.5, speed_cap)
            elif key == "steering_angle":
                clipped[:, idx] = np.clip(clipped[:, idx], -0.4189, 0.4189)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.agent_states = []
        self.prev_v = []
        self.prev_agent_steer = [0.0 for _ in range(self.num_agents)]
        for i in range(self.num_agents):
            v = float(
                np.sqrt(
    ...
    def step(self, patch_action):
        ...
        if self.cfg.coupling_enabled and self.agent_states is not None:
            local_x = []
            for i in range(self.num_agents):
