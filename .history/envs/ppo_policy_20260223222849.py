...
@dataclass
class PatchEnvConfig:
    # Keep only essential configs (match single-agent)
    ...
    # Patch size (FIXED - no control)
    patch_a: float = 1.5  # Fixed size
    patch_b: float = 1.0  # Fixed size
...
class PatchEnv(gym.Env):
    def __init__(self, config: Optional[PatchEnvConfig] = None):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf, 
            high=np.inf, 
            shape=(4 + self.cfg.num_beams,),  # Match single-agent
            dtype=np.float32
        )
        
        # Base environment (simplified)
        self.f110 = F110EnvAdapter(
            F110Config(
                num_agents=0,  # No agents needed
                reset_type=self.cfg.base_reset_type,
                control_input=("speed", "steering_angle"),
            ...
            render_mode=self.cfg.render_mode,
                ...
                # Patch with FIXED size
        self.patch = DynamicPatch()
        self.patch.a = self.cfg.patch_a  # Fixed
        self.patch.b = self.cfg.patch_b  # Fixed
        
        # Frenet bookkeeping (same as single-agent)
        self.track_spline = None
        self.track_length = None
        self.prev_s = None
        self.prev_steer = 0.0
