...
class LidarModel:
    ...
    def aggregate(self, obs: Dict, patch) -> np.ndarray:
        ...
        if np.nanmax(scans) <= 1.5:
            scans = scans * self.config.max_range_m
        ...
        # Use max-range defaults to avoid inf values leaking into observations.
        aggregated = np.ones(n_sectors, dtype=np.float32) * self.config.max_range_m
        ...
        aggregated = np.nan_to_num(
            aggregated,
            nan=self.config.max_range_m,
            posinf=self.config.max_range_m,
            neginf=0.0,
        )
        return np.clip(aggregated, 0.0, self.config.max_range_m).astype(np.float32)
    ...
    def check_lidar_safety(self, base_obs: Dict, patch, safety_margin: float = 0.5):
        ...
        scans = np.array(base_obs["scans"], dtype=np.float32)
        if np.nanmax(scans) <= 1.5:
            scans = scans * self.config.max_range_m
        scans = np.clip(scans, 0.0, self.config.max_range_m)
        scans = np.where(np.isfinite(scans), scans, self.config.max_range_m)
