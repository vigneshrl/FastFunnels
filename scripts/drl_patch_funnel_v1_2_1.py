...
class PatchFunnelEnvV1(gym.Env):
    ...
    def _compute_reward(self, base_obs):
        ...
        if progress_delta > 0:
            reward += 50.0 * progress_delta * 100
        ...
        reward -= 0.2 * min(total_outside_dist / 5.0, 2.0)
        ...
        if min_wall_dist < 1.0:
            reward -= 2.0 * (1.0 - min_wall_dist)  # Up to -7.5 when touching wall
        elif min_wall_dist < 2.0:
            reward -= 0.5 * (2.0 - min_wall_dist)  # Soft penalty zone
...
def run_eval_with_plots(model_path, num_episodes=5, save_plots=True):
    """Evaluate model and generate performance plots.
    
    Generates plots for:
    - Patch velocity over time
    - Containment rate over time
    - Lap progress over time
    - Cumulative reward over time
    - Agent distances from patch center
    """
    import matplotlib.pyplot as plt
    from collections import defaultdict
    
    print("=" * 70)
    print("DRL-GUIDED CONSTRAINT FUNNELING - EVALUATION WITH PLOTS")
    print(f"Loading model: {model_path}")
    print("=" * 70)
    ...
    env = PatchFunnelEnvV1(
        num_agents=2, 
        render_mode=None,  # No rendering for faster eval
        domain_randomize=False
    )
    ...
    for ep in range(num_episodes):
        ...
        episode_metrics = {
            ...
            'cumulative_reward': [],
            'instant_reward': [],
            'agent1_dist_from_center': [],
            'agent2_dist_from_center': [],
            'min_wall_clearance': [],
            'patch_heading': [],
        }
        ...
        while True:
            ...
            # Get internal env state (unwrap DummyVecEnv -> VecNormalize -> actual env)
            inner_env = vec_env.envs[0]
            if hasattr(inner_env, 'env'):
                inner_env = inner_env.env
            
            # Log metrics
            episode_metrics['timestep'].append(step)
            episode_metrics['patch_velocity'].append(inner_env.patch.v if hasattr(inner_env, 'patch') else 0)
            episode_metrics['patch_x'].append(inner_env.patch.x if hasattr(inner_env, 'patch') else 0)
            episode_metrics['patch_y'].append(inner_env.patch.y if hasattr(inner_env, 'patch') else 0)
            episode_metrics['patch_heading'].append(inner_env.patch.theta if hasattr(inner_env, 'patch') else 0)
            episode_metrics['lap_progress'].append(info_dict.get('lap_progress', 0))
            episode_metrics['cumulative_reward'].append(cumulative_reward)
            episode_metrics['instant_reward'].append(reward[0])
            
            # Containment
            agents_inside = info_dict.get('agents_inside', 0)
            total_agents = 2
            episode_metrics['containment_rate'].append(agents_inside / total_agents)
            
            # Agent distances from patch center
            if hasattr(inner_env, 'agent_states') and hasattr(inner_env, 'patch'):
                for i, agent_state in enumerate(inner_env.agent_states):
                    ax, ay = agent_state[0], agent_state[1]
                    px, py = inner_env.patch.x, inner_env.patch.y
                    dist = np.sqrt((ax - px)**2 + (ay - py)**2)
                    if i == 0:
                        episode_metrics['agent1_dist_from_center'].append(dist)
                    elif i == 1:
                        episode_metrics['agent2_dist_from_center'].append(dist)
            
            # Wall clearance
            episode_metrics['min_wall_clearance'].append(info_dict.get('patch_current_clearance', 0))
            
            if done[0]:
                print(f"  Episode {ep+1} finished at step {step}")
                print(f"    Final progress: {info_dict.get('lap_progress', 0):.1%}")
                print(f"    Total reward: {cumulative_reward:.1f}")
                print(f"    Termination: {info_dict.get('termination_reason', 'unknown')}")
                break
        
        all_episode_data.append(episode_metrics)
    ...
    fig.suptitle('DRL Patch Funnel Evaluation Metrics', fontsize=14, fontweight='bold')
    
    colors = plt.cm.tab10(np.linspace(0, 1, num_episodes))
    
    # Plot 1: Patch Velocity over Time
    ax1 = axes[0, 0]
    for i, ep_data in enumerate(all_episode_data):
        ax1.plot(ep_data['timestep'], ep_data['patch_velocity'], 
                 color=colors[i], alpha=0.7, label=f'Ep {i+1}')
    ax1.set_xlabel('Timestep')
    ax1.set_ylabel('Patch Velocity (m/s)')
    ax1.set_title('Patch Velocity Over Time')
    ax1.legend(loc='upper right', fontsize=8)
    ax1.grid(True, alpha=0.3)
    
    # Plot 2: Containment Rate over Time
    ax2 = axes[0, 1]
    for i, ep_data in enumerate(all_episode_data):
        ax2.plot(ep_data['timestep'], ep_data['containment_rate'], 
                 color=colors[i], alpha=0.7, label=f'Ep {i+1}')
    ax2.set_xlabel('Timestep')
    ax2.set_ylabel('Containment Rate')
    ax2.set_title('Agents Inside Patch (Containment Rate)')
    ax2.set_ylim(-0.1, 1.1)
    ax2.axhline(y=1.0, color='green', linestyle='--', alpha=0.5, label='Full containment')
    ax2.legend(loc='lower right', fontsize=8)
    ax2.grid(True, alpha=0.3)
    
    # Plot 3: Lap Progress over Time
    ax3 = axes[1, 0]
    for i, ep_data in enumerate(all_episode_data):
        ax3.plot(ep_data['timestep'], [p * 100 for p in ep_data['lap_progress']], 
                 color=colors[i], alpha=0.7, label=f'Ep {i+1}')
    ax3.set_xlabel('Timestep')
    ax3.set_ylabel('Lap Progress (%)')
    ax3.set_title('Lap Progress Over Time')
    ...
    ax4 = axes[1, 1]
    for i, ep_data in enumerate(all_episode_data):
        ax4.plot(ep_data['timestep'], ep_data['cumulative_reward'], 
                 color=colors[i], alpha=0.7, label=f'Ep {i+1}')
    ...
    # Plot 5: Agent Distances from Patch Center
    ax5 = axes[2, 0]
    for i, ep_data in enumerate(all_episode_data):
        if ep_data['agent1_dist_from_center']:
            ax5.plot(ep_data['timestep'], ep_data['agent1_dist_from_center'], 
                     color=colors[i], alpha=0.5, linestyle='-', label=f'Ep {i+1} Ag1')
            ax5.plot(ep_data['timestep'], ep_data['agent2_dist_from_center'], 
                     color=colors[i], alpha=0.5, linestyle='--', label=f'Ep {i+1} Ag2')
    ax5.set_xlabel('Timestep')
    ax5.set_ylabel('Distance from Patch Center (m)')
    ax5.set_title('Agent Distance from Patch Center')
    ax5.grid(True, alpha=0.3)
    # Don't show legend here - too cluttered
    
    # Plot 6: Patch Trajectory (X-Y)
    ax6 = axes[2, 1]
    for i, ep_data in enumerate(all_episode_data):
        ax6.plot(ep_data['patch_x'], ep_data['patch_y'], 
                 color=colors[i], alpha=0.7, label=f'Ep {i+1}')
        # Mark start and end
        ax6.scatter(ep_data['patch_x'][0], ep_data['patch_y'][0], 
                    color=colors[i], marker='o', s=50)
        ax6.scatter(ep_data['patch_x'][-1], ep_data['patch_y'][-1], 
                    color=colors[i], marker='x', s=50)
    ax6.set_xlabel('X Position (m)')
    ax6.set_ylabel('Y Position (m)')
    ax6.set_title('Patch Trajectory (o=start, x=end)')
    ...
    if save_plots:
        ...
        plot_path = os.path.join(plot_dir, 'evaluation_plots.png')
        plt.savefig(plot_path, dpi=150, bbox_inches='tight')
        print(f"\n✅ Plots saved to: {plot_path}")
        
        # Also save as PDF for papers
        pdf_path = os.path.join(plot_dir, 'evaluation_plots.pdf')
    ...
    # Aggregate stats
    final_progress = [ep['lap_progress'][-1] for ep in all_episode_data]
    final_rewards = [ep['cumulative_reward'][-1] for ep in all_episode_data]
    avg_velocities = [np.mean(ep['patch_velocity']) for ep in all_episode_data]
    avg_containment = [np.mean(ep['containment_rate']) for ep in all_episode_data]
    
    print(f"Lap Progress:      {np.mean(final_progress)*100:.1f}% ± {np.std(final_progress)*100:.1f}%")
    print(f"Total Reward:      {np.mean(final_rewards):.1f} ± {np.std(final_rewards):.1f}")
    print(f"Avg Patch Speed:   {np.mean(avg_velocities):.2f} ± {np.std(avg_velocities):.2f} m/s")
    print(f"Avg Containment:   {np.mean(avg_containment)*100:.1f}% ± {np.std(avg_containment)*100:.1f}%")
    print("=" * 70)
    
    return all_episode_data
# ============================================================================
# 5. DEMO WITH HEURISTIC POLICY
...
Examples:
  ...
  python3 drl_patch_funnel_v1.py --mode eval_plots --model path/to/model
        """
    )
    parser.add_argument("--mode", choices=["demo", "train", "eval", "eval_plots"], default="demo")
