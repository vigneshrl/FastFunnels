...
try:
    from envs.centerline import CenterlineFrame
except ImportError:  # executed as a loose module
    from centerline import CenterlineFrame


def _reproject_onto_centerline(spline, L, px, py, cache_holder):
    """Dense nearest-point projection of (px, py) onto `spline` over [0, L].

    Used when f1tenth_gym's `cartesian_to_frenet` returns s > L because it
    projected the point onto the PHANTOM closing segment (the straight B->A
    line the gym appends to open tracks). The previous fix snapped such
    readings to whichever real endpoint was closer, which is wrong on
    serpentine maps (mf_zigzag): a car at true ~50% progress sits near y=0,
    i.e. right on the phantom line, and gets snapped to s=L -> a bogus
    "arrived". Re-projecting onto the real centerline gives the true s/ey.

    Returns (s, ey_signed). Caches the dense samples on `cache_holder`.
    """
    samp = getattr(cache_holder, "_frenet_dense", None)
    if samp is None or abs(samp[0] - L) > 1e-3:
        n = max(400, int(L / 0.25))
        ss = np.linspace(0.0, L, n)
        pts = np.array([spline.calc_position(float(u)) for u in ss], dtype=np.float64)
        samp = (L, ss, pts)
        cache_holder._frenet_dense = samp
    _, ss, pts = samp
    d2 = (pts[:, 0] - px) ** 2 + (pts[:, 1] - py) ** 2
    k = int(np.argmin(d2))
    k0, k1 = max(0, k - 1), min(len(ss) - 1, k + 1)
    tx, ty = pts[k1, 0] - pts[k0, 0], pts[k1, 1] - pts[k0, 1]
    nrm = float(np.hypot(tx, ty)) or 1.0
    ey = ((px - pts[k, 0]) * (-ty) + (py - pts[k, 1]) * tx) / nrm
    return float(ss[k]), float(ey)
...
class PatchEnvConfig:
    ...
    # Action repeat: hold each policy action for this many sim steps.
    # The f1tenth steering actuator slews at sv_max=3.2 rad/s -> a full
    # +/-0.4189 sweep takes 0.26 s = 26 sim steps. A 100 Hz (K=1) policy makes
    # ~26 decisions per steering movement, so the reward-optimal control is
    # bang-bang (command sign = "slew this way", magnitude meaningless) -- 7
    # training runs of steering-penalty tuning could not remove it. K=5 -> the
    # Back to 1: the steering-as-RATE reparam (see steer_rate_max + step())
    # makes bang-bang impossible on its own, so action-repeat is no longer
    # needed and K=1 keeps full training throughput.
    action_repeat: int = 1
    # Steering-as-rate: action[0] in [-1,1] is a steering RATE. Each policy step
    # the commanded angle moves by at most steer_rate_max rad, integrated into a
    # persisted state. 0.03 rad/step at 100 Hz = 3.0 rad/s, just under the
    # sv_max=3.2 actuator limit -> the command is always physically trackable
    # and a full +/-0.4189 sweep takes ~28 steps (0.28 s). The policy CANNOT
    # produce a bang-bang command -- the swerve is fixed at the source.
    steer_rate_max: float = 0.03
    # LEGACY MODE (added 2026-09-06): reproduce the June-2026 checkpoint's raw
    # action space and simple reward exactly, for a controlled comparison
    # against today's "lean reward" (v_geo cap, obstacle-aware room_half/
    # overwidth, size_gain gating) on the hypothesis that the lean reward's
    # extra machinery -- not RL/PPO itself -- is the source of the loiter/
    # crawl local optima seen since Aug. Reintroduces the pre-fix bang-bang
    # steering risk (steering-as-rate was what fixed that, unrelated to
    # reward) -- an accepted, explicit tradeoff for this experiment, not a
    # recommended default.
    #
    # legacy_action_space=True: action_space becomes the raw June Box
    # ([-0.4189,0.4189] absolute steer rad, [2,10] speed m/s, [a_cmd_min,max],
    # [b_cmd_min,max]) decoded directly in step() with NO steering-rate
    # integrator (see PatchCarEnv.step()).
    legacy_action_space: bool = False
    # legacy_reward=True: _compute_reward_for uses the exact formula recovered
    # from git commit d750295 (recorded in patch_policy_models/best_patch/
    # config.yaml) instead of today's reward -- unconditional fill_ratio
    # (b_now/half_w, no room_half/size_gain), unconditional speed^2 (no v_geo
    ...
    # 0.6 (run 12 tried 0.3: weakening this let the policy wander/circle near
    # the spawn -- the loiter exploit. 1.0 was too strong against the
    # obstacle go-around (run 10). 0.6 splits the difference; the real
    # go-around signal now comes from reward_overwidth_weight, not from
    # relaxing this.
    reward_crosstrack_weight: float = 0.6
    # E (2026-09-02, run 7): the trace showed steering pinned at +/-0.4189 (max
    # lock) ~80% of steps, flipping sign, even in wide open corridor -- a
    # bang-bang policy. This is what wrecks open_narrow (ey flung 0.07->0.93 at
    # the tunnel mouth) and makes the funnel unfollowable for the MPC. Run 6
    # bumped the penalties to 0.08 / 0.30 but tested via RESUME, which never
    # moves a locked-in behaviour -- no effect. Run 7 tests from scratch with a
    # much harder penalty: |steer| itself is now penalised (breaks sitting on
    # the rail) AND a +/-max flip (steer_rate ~84 rad/s) costs ~21/step.
    # Legit slalom/zigzag steering (rate <~0.5 rad/s, |steer| <~0.15) stays cheap.
    # action_repeat=3 (33 Hz) is the primary swerve fix -- at that rate bang-bang
    # steering is no longer reward-optimal. These are LIGHT nudges only: the
    # hard 0.4 / 0.15 version made a fresh policy refuse to steer -> instant
    # crashes on every curved map -> 80-step episodes -> reset/map-swap overhead
    # tanked throughput to ~230 steps/s. Re-add weight only if K=3 still swerves.
    reward_steer_bias_weight: float = 0.05   # penalise |steer|, breaks max-steer local optimum
    reward_steer_rate_weight: float = 0.05
    ...
    # --- geometry-referenced cruise speed --------------------------------
    # The patch kept converging to "crawl at the floor speed through the narrow
    # section, survive" — a local optimum, since crawling both banks per-step
    # reward and avoids the near-term crash the collision penalty guards. This
    # term pays a bonus for MATCHING the fastest speed the local corridor
    # curvature and width physically allow, so driving slower than the geometry
    # permits now leaves reward on the table.
    reward_vtrack_weight: float = 3.0
    vtrack_alat_max: float = 6.0     # m/s^2 lateral-accel budget -> curvature speed cap
    vtrack_v_cap: float = 8.0        # m/s absolute ceiling
    vtrack_v_floor: float = 3.0      # slowest speed ever demanded (widest + straightest)
    vtrack_width_ref: float = 2.5    # half-width (m) at/above which the width cap reaches v_cap
    # penalty for exceeding v_geo, so the unbounded progress reward can't be
    # gamed by sprinting into every wall ("as fast as geometry allows, not faster").
    # 6.0: the progress/speed reward is also capped at v_geo pace now, so
    # overspeed has zero upside -- this penalty just needs to be a firm nudge,
    # not fight an uncapped progress term (which it lost at 3 and at 10).
    reward_overspeed_weight: float = 6.0
    # --- obstacle-aware sizing / speed (map-derived, not lidar) --------------
    # The env knows the map: `_precompute_passage_widths` samples the widest
    # free lateral span (the drivable gap) along the centreline, which captures
    # both wall pinches AND mid-corridor obstacles. The reward then (a) sizes
    # the funnel to the tightest passable gap in the next few metres, (b) slows
    # for it, (c) penalises the ellipse being wider than that gap. This is what
    # was missing in run 10 -- the policy kept b~3.9 straight into a 3 m gap.
    pass_lookahead_m: tuple = (2.0, 5.0, 8.0, 11.0)
    # Run A (2026-09-04): expose the SAME map-derived gap profile used in the
    # reward as OBSERVATION features too -- (gap width, gap centre offset) at
    # each pass_lookahead_m distance, so the policy can start easing over
    # early instead of reacting once lidar finally picks the obstacle up at
    # ~3 m. Default False so nothing that re-imports this module with the old
    # config (obs_dim 115) breaks; opt in via PatchEnvConfig(obs_gap_lookahead=True)
    # or env var PATCH_GAP_LOOKAHEAD=1 (see make_patch_env).
    obs_gap_lookahead: bool = False
    reward_overwidth_weight: float = 2.0   # penalty for b > (passable half-gap ahead)
    ...
    reward_speed_steer_weight: float = 0.1   # was 0.3 — over-suppressed speed in every curve
    stuck_no_progress_steps: int = 100
    stuck_progress_eps: float = 1e-3 #5e-4
    # 150 (was 10): the joint 5-map run learned a new "freeze in front of any
    # constriction" local optimum -- at -10 vs the -3000 collision penalty,
    # stopping before a pinch was 300x cheaper than a failed attempt. Freezing
    # must cost enough to make a careful attempt the better bet, without going
    # so high the policy revives creeping (capped progress + vtrack reward keep
    # creeping worthless anyway).
    stuck_penalty: float = 150.0
    ...
    # F (2026-09-01): 3.0 was a flat cap regardless of corridor width -> in a
    # 9.25 m corridor (half-width 4.6) the funnel maxed at 6.0 m = 65% fill,
    # ever. step() already caps the realised b at near_wall_dist*0.85 (which
    # DOES scale with the corridor), so raising this ceiling lets that
    # geometry-aware term bind instead: b now grows to ~4.0 in the widest
    # sections (85% of half-width) and the existing filter still clamps it down
    # at every pinch.
    b_cmd_max: float = 4.5
    a_cmd_min: float = 1.5
...
class JointEnv:
    ...
    def _patch_to_frenet(self, patch=None) -> tuple:
        ...
        if cl is not None:
            return cl.project(float(p.x), float(p.y))
        ...
        try:
            ...
            # Open-track fix: f1tenth_gym's coarse nearest-point search can
            # project a point near the start onto the PHANTOM closing segment
            # (the straight line the gym appends from B back to A). That returns
            # s in (track_length, spline.s[-1]] -- e.g. s~219 for a car 1 m past
            # A on this map -- which spuriously trips the `s >= track_length`
            # "arrived" check and dumps the lap-finish bonus into the reward.
            # Re-project any phantom-segment reading (s > L) onto the real
            # centerline -- see _reproject_onto_centerline for why the old
            # nearest-endpoint snap was wrong on serpentine maps.
            L = float(self.track_length) if self.track_length else 0.0
            if L > 0.0 and s > L and self.track_spline is not None:
                s, ey = _reproject_onto_centerline(
                    self.track_spline, L, float(p.x), float(p.y), self)
    ...
    def _build_patch_obs(self, base_obs) -> np.ndarray:
        ...
        track_yaw = float(self.cl.yaw_at(float(s)))
        ...
        curvature = float(self.cl.curvature_at(float(s)))
        ...
        if self.cfg.obs_mode == "lidar":
            ...
            curvature_ahead = [
                float(self.cl.curvature_at(min(float(s) + (i + 1) * 2.0, L - 1e-3)))
                for i in range(10)
            ]
            width_ahead = [
    ...
    def reset(self, seed=None, options=None):
        ...
        self.track = track
        self.track_spline = track.centerline.spline
        self.cl = CenterlineFrame(self.track_spline.points[:, 0], self.track_spline.points[:, 1])  # see envs/centerline.py
        if self.cfg.open_track:
            self.track_length = float(self.cl.L)
        else:
            self.track_length = float(self.track_spline.s[-1])
        ...
        # spawn on the CLEAN centreline frame (track.centerline.xs is the gym's
        # spline-resampled path, which is garbage on open tracks)
        xs = np.asarray(self.cl.x, dtype=np.float32)
        ys = np.asarray(self.cl.y, dtype=np.float32)
        max_spawn_idx = max(1, xs.shape[0] - 2)
...
class PatchEnv(gym.Env):
    ...
    def __init__(self, config: Optional[PatchEnvConfig] = None):
        ...
        if self.cfg.legacy_action_space:
            # Raw physical ranges, exactly as the June checkpoint used (see
            # PatchEnvConfig.legacy_action_space) -- reintroduces the bang-bang
            # susceptibility the centred+rate reparam above was built to fix.
            self.action_space = spaces.Box(
                low=np.array([-0.4189, 2.0, self.cfg.a_cmd_min, self.cfg.b_cmd_min],
                              dtype=np.float32),
                high=np.array([0.4189, 10.0, self.cfg.a_cmd_max, self.cfg.b_cmd_max],
                              dtype=np.float32),
                dtype=np.float32,
            )
        else:
        ...
        if self.cfg.obs_mode == "lidar":
            ...
            if self.cfg.obs_gap_lookahead:
                _obs_dim += 2 * len(self.cfg.pass_lookahead_m)
        else:
            _obs_dim = 22 + 64
        self.observation_space = spaces.Box(
            low=-np.inf,
    ...
    def _precompute_passage_widths(self, n_samples: int = 240) -> None:
        """Widest free lateral span (the drivable GAP) along the centreline.

        Perpendicular scan of the occupancy map at each s: a mid-corridor
        obstacle occupies the centreline pixel but leaves free space to one
        side -- this measures the largest contiguous free run, i.e. the real
        gap the funnel must fit through. Captures wall pinches AND obstacles.
        Map is known, so this is exact and computed once per reset.
        """
        self._pass_s_vals = None
        self._pass_w = None
        self._pass_off = None
        if self._occ_map is None or self.track_spline is None:
            return
        occ = self._occ_map                       # 1.0 = free, 0.0 = wall
        res = float(self._resolution)
        ox, oy = float(self._origin[0]), float(self._origin[1])
        H, W = occ.shape
        s_vals = np.linspace(0.0, float(self.track_length), n_samples, endpoint=False)
        ...
        for i, s in enumerate(s_vals):
            try:
                x, y = self.cl.position_at(float(s))
                yaw = float(self.cl.yaw_at(float(s)))
                nx, ny = -np.sin(yaw), np.cos(yaw)
                ts = np.arange(-6.0, 6.0 + 1e-6, res)
                cx = np.clip(((x + ts * nx - ox) / res).astype(int), 0, W - 1)
                cy = np.clip(((y + ts * ny - oy) / res).astype(int), 0, H - 1)
                free = (occ[cy, cx] > 0.5).astype(np.int8)
                # longest contiguous run of free cells, vectorised
                edges = np.flatnonzero(np.diff(np.concatenate(([0], free, [0]))))
                ...
                if starts.size:
                    ...
                    # along the same left-positive normal as ey_signed
                    mid = 0.5 * (starts[k] + ends[k] - 1)
                    goff[i] = -6.0 + mid * res
            except Exception:
                pass
    ...
    def _lookup_pass_w(self, s: float) -> float:
        ...
        if getattr(self, "_pass_s_vals", None) is None:
            return 2.0 * self._lookup_half_w(s)
        s_wrapped = float(s) % float(self.track_length)
    ...
    def _lookup_gap_offset(self, s: float) -> float:
        ...
        drivable gap's centre from the centreline -- which SIDE to steer for,
        map-derived (not lidar). Falls back to 0 (assume centred) if uncomputed."""
        if getattr(self, "_pass_off", None) is None:
            return 0.0
        s_wrapped = float(s) % float(self.track_length)
        idx = int(np.searchsorted(self._pass_s_vals, s_wrapped))
        idx = min(idx, len(self._pass_off) - 1)
        return float(self._pass_off[idx])
    ...
    def _get_current_scan(self) -> np.ndarray:
        ...
        raw = np.clip(raw, 0.0, float(getattr(self.cfg, 'lidar_clip_m', 30.0)))
    ...
    def _build_obs_for(self, patch: "DynamicPatch", lane_id: int) -> np.ndarray:
        ...
        if self.cfg.obs_mode == "lidar":
            ...
            core = [s_norm, ey_signed, speed, psi_error, a, b, curvature]
            if self.cfg.obs_gap_lookahead:
                # map-derived (NOT lidar): drivable-gap width + signed centre
                # offset at each lookahead distance -- same ground truth the
                # reward's room_half/overwidth terms already use, now given to
                # the policy directly so it can act on it early.
                for d in self.cfg.pass_lookahead_m:
                    sd = min(float(s) + d, L - 0.01)
                    core.append(self._lookup_pass_w(sd))
                    core.append(self._lookup_gap_offset(sd))
            scalars = np.array(core, dtype=np.float32)  # (7,) or (7+2*len(pass_lookahead_m),)
    ...
    def _patch_to_frenet(self, patch: "DynamicPatch") -> tuple[float, float]:
        cl = getattr(self, "cl", None)
        if cl is not None:
            return cl.project(float(patch.x), float(patch.y))
        if self.track is None or not hasattr(self.track, 'cartesian_to_frenet'):
            return 0.0, 0.0
        try:
            s, ey, _ = self.track.cartesian_to_frenet(
            ...
            # Open-track fix: f1tenth_gym's coarse nearest-point search can
            # project a point near the start onto the PHANTOM closing segment
            # (straight line from B back to A the gym appends). That returns
            # s in (track_length, spline.s[-1]] -- e.g. s~219 for a car 1 m past
            # A on open_narrow -- which spuriously trips the `s >= track_length`
            # "arrived" check and dumps the lap-finish bonus into the reward.
            # Re-project any phantom-segment reading (s > L) onto the real
            # centerline -- see _reproject_onto_centerline for why the old
            # nearest-endpoint snap was wrong on serpentine maps.
            ...
            if L > 0.0 and s > L and self.track_spline is not None:
                s, ey = _reproject_onto_centerline(
                    self.track_spline, L, float(patch.x), float(patch.y), self)
            return float(s), float(ey)
        except (AttributeError, TypeError):
            return 0.0, 0.0
    ...
    def _compute_reward_for(
        ...
        # --- obstacle-aware room (map-derived, not lidar) -------------------
        # Tightest DRIVABLE gap over the next few metres: captures wall pinches
        # AND mid-corridor obstacles. `room_half` is the half-space the funnel
        # actually has -- the smaller of the track half-width and this gap.
        _Lt = max(float(self.track_length), 1e-3)
        _pass_ahead = min(
            self._lookup_pass_w(min(float(s) + d, _Lt - 0.01))
        ...
        # there instead of ploughing in at b~4).
        fill_ratio = float(np.clip(b_now / room_half, 0.0, 1.0))
        ...
        # SYMMETRIC around ds=0 (fix, 2026-09-05) -- previously floored at
        # max(ds, 0.0), which made a backward step cost nothing here while a
        # forward step of the same size paid in full. That let the policy
        # rock in place (alternating +ds/-ds) and collect this bonus on every
        # forward half-swing for free: confirmed on run_20260904_190247's
        # best_model, +647.6 net reward over 6000 steps on mf_slalom while
        # oscillating between s=8.8m and s=10.5m of a 153m track (going
        # nowhere). Clipping ds itself (not max(ds,0)) makes a backward step
        # cost exactly what a forward step of that size would have earned --
        # an oscillation nets ~0 per swing instead of "backward is free".
        # A forward-only trajectory (ds >= 0 every step, i.e. any competent
        # driving) is mathematically IDENTICAL to the old formula: the clip's
        # negative branch never engages. Verified on two known-stalled
        # rollouts (mf_slalom, on_rep_obs): size+speed payout drops 90% / 62%.
        ds_nominal = max(self.cfg.reward_size_speed_ref * float(dt), 1e-9)
        size_gain = float(np.clip(ds / ds_nominal, -1.0, 1.0))

        # --- geometry-referenced cruise speed -------------------------------
        # v_geo = fastest speed the local corridor permits: min of the
        # curvature limit sqrt(a_lat / kappa) and a width-based cap. The bonus
        # peaks when speed_now ~ v_geo and decays either side, gated on forward
        # progress so a standstill still earns nothing.
        _kap = abs(float(self.cl.curvature_at(float(s))))
        _v_curve = (self.cfg.vtrack_v_cap if _kap < 1e-4
                    else float(np.sqrt(self.cfg.vtrack_alat_max / _kap)))
        _w_span = max(self.cfg.vtrack_width_ref - 1.0, 1e-3)
        # width cap uses room_half (obstacle-aware) so v_geo drops when the
        # drivable gap ahead narrows, not just when the track walls close in
        _v_width = self.cfg.vtrack_v_floor + (
            self.cfg.vtrack_v_cap - self.cfg.vtrack_v_floor) * float(
            np.clip((room_half - 1.0) / _w_span, 0.0, 1.0))
        v_geo = float(np.clip(min(_v_curve, _v_width),
                              self.cfg.vtrack_v_floor, self.cfg.vtrack_v_cap))
        # v_track_match (a positive bonus for |speed-v_geo| regardless of net
        # direction, gated only by instantaneous max(ds,0)) was the loiter
        # exploit's fuel in run 12: circling forward-ish paid it every lap
        # with ~zero net progress. DROPPED from reward_raw (kept only in the
        # logged terms dict, for diagnostics). Speed-matching pressure now
        # comes only from the progress cap + overspeed penalty below (both
        # proven since run 4 -- never rail, never crawl -- and neither is
        # farmable by looping since they depend on net `ds`, not instantaneous
        # motion).
        v_track_match = max(0.0, 1.0 - abs(speed_now - v_geo) / max(v_geo, 1e-3))
        # Speed bonus's ds_cap: same symmetric fix as size_gain above (clip
        # ds itself instead of flooring at 0) -- _prog (the progress term)
        # was already symmetric (no floor), so it was never the exploitable
        # half of this reward.
        _ds_cap = float(np.clip(ds, -v_geo * float(dt), v_geo * float(dt)))
        _prog = (min(ds, v_geo * float(dt)) if ds > 0.0 else ds)

        # --- LEGACY fill_ratio (b_now/half_w, NOT obstacle-aware) -- June never
        # trained on obstacles, so its fill_ratio only ever knew the track's
        # own half-width, never the drivable-gap `room_half` above. Computed
        ...
        if self.cfg.legacy_reward:
            ...
            # to test the hypothesis that today's "lean reward" machinery
            # (v_geo cap, room_half/overwidth, size_gain gating), not PPO/RL
            # itself, is the source of the loiter/crawl local optima seen
            # since Aug. Every term here is UNCONDITIONAL: raw ds (no v_geo
            # cap), raw speed^2 (no v_geo match), raw fill_ratio (no
            # obstacle-aware sizing, no size_gain progress gate) -- this is
            # deliberately the simpler, less-hardened reward, on the theory
            # that PPO's own exploration/exploitation may avoid the
            ...
            reward_raw = (
                ...
                - self.cfg.shape_area_penalty_weight * area_excess
                - self.cfg.time_penalty_per_sec * float(dt)
                # ck18510000 recipe (wandb diff.patch of run_20260518_151612, also June's):
                #   lap bonus DISABLED, speed*|steer| penalty ADDED, speed bonus gated on forward ds
            ...
            # size/overspeed + light steer-rate + crosstrack RESTORED to 0.6 (was
            # weakened to 0.3 for run 12, which let the policy wander/loiter).
            # Dropped entirely: v_track_match (above), reward_steer_bias_weight,
            # reward_spin_weight, shape_area_penalty_weight, time_penalty_per_sec,
            # reward_speed_steer_weight -- none were pulling their weight and each
            ...
            reward_raw = (
                ...
                + float(lap_bonus)
                + self.cfg.reward_size_weight * fill_ratio * size_gain          # obstacle-aware fill (room_half)
                + self.cfg.reward_speed_weight * min(speed_now, v_geo) * _ds_cap  # forward progress, capped at geo pace
                - self.cfg.reward_overspeed_weight * max(0.0, speed_now - v_geo)  # ...but not faster
                - self.cfg.reward_overwidth_weight * max(0.0, b_now - room_half)  # funnel wider than the drivable gap ahead
            )
        if no_progress >= self.cfg.stuck_no_progress_steps:
            reward_raw -= self.cfg.stuck_penalty
        ...
        terms = {
            ...
            "yaw_rate": float(yaw_rate),
            "spin_excess": float(spin_excess),
            "v_geo": float(v_geo), "speed_now": float(speed_now),
            "room_half": float(room_half), "pass_ahead": float(_pass_ahead),
            "b_now": float(b_now),
            "v_track_match": float(v_track_match),
            # --- per-component reward breakdown (diagnostic only, does not
            # affect reward_raw above) -- added 2026-09-05 to trace the
            # loiter/crawl exploit: which term still pays on instantaneous
            # forward motion regardless of NET progress over an episode.
            "fill_ratio": float(fill_ratio),
            "legacy_fill_ratio": float(legacy_fill_ratio),
            "size_gain": float(size_gain),
            "term_progress": float(self.cfg.reward_progress_scale * _prog),
            "term_crosstrack": float(-self.cfg.reward_crosstrack_weight * abs(ey)),
            "term_steer_rate": float(-self.cfg.reward_steer_rate_weight * steer_rate),
            "term_collision": float(-self.cfg.collision_penalty if collision else 0.0),
            "term_size": float(self.cfg.reward_size_weight * fill_ratio * size_gain),
            "term_speed": float(self.cfg.reward_speed_weight * min(speed_now, v_geo) * _ds_cap),
            "term_overspeed": float(-self.cfg.reward_overspeed_weight * max(0.0, speed_now - v_geo)),
            "term_overwidth": float(-self.cfg.reward_overwidth_weight * max(0.0, b_now - room_half)),
        }
    ...
    def reset(self, seed=None, options=None):
        ...
        try:   # v3_x5: the lidar's reach scales with the corridor
            from f1tenth_gym.envs.base_classes import RaceCar as _RC
            if _RC.scan_simulator is not None:
                _RC.scan_simulator.max_range = float(getattr(self.cfg, 'lidar_clip_m', 30.0))
        except Exception:
            pass
        if track.centerline is None or track.centerline.spline is None:
        ...
        self.track = track
        self.track_spline  = track.centerline.spline
        # Env-owned Frenet frame from the RAW csv waypoints (spline.xs/ys).
        # The gym's periodic spline is unusable on open tracks -- see
        # envs/centerline.py. All geometry below (frenet, yaw, curvature,
        # width sampling, spawn) goes through self.cl.
        self.cl = CenterlineFrame(self.track_spline.points[:, 0], self.track_spline.points[:, 1])
        if self.cfg.open_track:
            self.track_length = float(self.cl.L)
        else:
            self.track_length = float(self.track_spline.s[-1])
        ...
        # Precompute the drivable-gap profile (obstacle-aware, map-derived).
        # 0.4 m sampling is plenty -- the reward lookahead is at 2-11 m spacing.
        self._precompute_passage_widths(
            n_samples=min(600, max(200, int(self.track_length / 0.4))))
        ...
        self._steer_state = 0.0   # steering-as-rate integrator (see step())
    ...
    def _visualize(self):
        ...
        # --- Redraw background imshow only when the view window shifts ---
        if occ_map is not None:
            margin = max(prim.a, prim.b) + 15 * getattr(self, "_view_scale", 1.0)   # eval-only zoom knob
            px_min = max(0, int((cx - margin - origin[0]) / resolution))
            px_max = min(occ_map.shape[1], int((cx + margin - origin[0]) / resolution))
            py_min = max(0, int((cy - margin - origin[1]) / resolution))
            py_max = min(occ_map.shape[0], int((cy + margin - origin[1]) / resolution))
        ...
        # --- View limits ---
        view_margin = max(prim.a, prim.b) + 10 * getattr(self, "_view_scale", 1.0)   # eval-only zoom knob
        ax.set_xlim(cx - view_margin, cx + view_margin)
        ax.set_ylim(cy - view_margin, cy + view_margin)
    ...
    def get_patch_action(self, patch: "DynamicPatch", track_spline) -> np.ndarray:
        ...
        if self.track_spline is None:
            ...
            self.cl = CenterlineFrame(track_spline.xs, track_spline.ys)
            self.track_length = float(self.cl.L)
...
class PatchCarEnv(PatchEnv):
    ...
    def step(self, action):
        self.step_count += 1
        _K = max(1, int(getattr(self.cfg, "action_repeat", 1)))
        dt = self.cfg.control_dt * _K   # effective control interval (action held for _K sim steps)
        lap_bonus = 0.0
        lap_time = None
        arrived   = False  # True when open-track car reaches point B

        if self.cfg.legacy_action_space:
            # Raw physical action, exactly as June's checkpoint used: no
            # centring, no steering-RATE integration -- action[0] IS the
            # absolute steering angle for this step (see
            # PatchEnvConfig.legacy_action_space; ground truth from git
            # d750295's PatchEnv.step()). This reintroduces the bang-bang
            # steering risk the rate reparam below exists to prevent.
            steering_cmd = float(np.clip(
                np.nan_to_num(action[0], nan=0.0, posinf=0.4189, neginf=-0.4189),
                -0.4189, 0.4189))
            self._steer_state = steering_cmd  # kept in sync for any code that reads it
            speed_cmd = float(np.clip(
                np.nan_to_num(action[1], nan=2.0, posinf=10.0, neginf=2.0),
                2.0, 10.0))
            ...
            # action[0] is a steering RATE in [-1, 1]: integrate into a persisted,
            # bounded steer state so the command moves at most steer_rate_max rad
            # per policy step -> bang-bang is physically impossible (a full sweep
            # takes ~28 steps). Fixes the swerve at the source.
            _da = float(np.clip(np.nan_to_num(action[0], nan=0.0), -1.0, 1.0)) * self.cfg.steer_rate_max
            self._steer_state = float(np.clip(
                getattr(self, "_steer_state", 0.0) + _da, -0.4189, 0.4189))
            steering_cmd = self._steer_state
        ...
        # Action repeat: hold [steering_cmd, speed_cmd] for _K sim steps so the
        # 20 Hz policy decision spans a real chunk of the slew-limited steering
        # actuator's response (see PatchEnvConfig.action_repeat).
        _act = np.array([[steering_cmd, speed_cmd]], dtype=np.float32)
        for _rep in range(_K):
            base_obs, _, _, _, _ = self.f110.step(_act)
            if "collisions" in base_obs and bool(
                np.asarray(base_obs["collisions"]).reshape(-1)[0] > 0.5
            ):
                break
    ...
    def _init():
        if map_pool_dir:
            ...
            try:
                ...
                _s.path.insert(0, os.path.join(_repo, "presentation_code"))
        ...
        _kw = dict(
            ...
            obs_gap_lookahead=(os.environ.get("PATCH_GAP_LOOKAHEAD", "0") == "1"),
        )
        # explicit per-run overrides (from --env-config yaml) win over the defaults above
        _kw.update({k: v for k, v in (env_overrides or {}).items()})
        cfg = PatchEnvConfig(**_kw)
        env = PatchCarEnv(cfg)
