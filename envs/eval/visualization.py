...
def evaluate_joint_policy(
    ...
    # all_agent_xs[i] / all_agent_ys[i] = list of per-episode trajectories for agent i
    all_agent_xs = [[] for _ in range(num_agents)]
    all_agent_ys = [[] for _ in range(num_agents)]
    all_agents_inside = []
    all_patch_speeds  = []
    ...
    for ep in range(num_episodes):
        ...
        ep_reward_patch  = 0.0
        patch_xs, patch_ys = [], []
        agent_xs = [[] for _ in range(num_agents)]
        agent_ys = [[] for _ in range(num_agents)]
        agents_inside_ts = []
        patch_speed_ts   = []
        ...
        while True:
            ...
            agent_obs = [env._step_obs[1 + i] for i in range(num_agents)]

            if patch_obs is None or any(o is None for o in agent_obs):
            ...
            agent_action_list = [
                agent_ppo.predict(_norm_agent(o), deterministic=True)[0]
                for o in agent_obs
            ]

            # --- step ---
            agent_actions = np.stack(
                [np.asarray(a, dtype=np.float32) for a in agent_action_list],
                axis=0,
            )
            ...
            ep_reward_agents += float(np.mean(
                [env._step_rewards[1 + i] for i in range(num_agents)]
            ))
            ...
            if env.current_base_obs is not None:
                ...
                for i in range(num_agents):
                    agent_xs[i].append(float(bobs["poses_x"][i]))
                    agent_ys[i].append(float(bobs["poses_y"][i]))
            ...
            if env._step_terminated or env._step_truncated:
                ...
                for i in range(num_agents):
                    all_agent_xs[i].append(agent_xs[i])
                    all_agent_ys[i].append(agent_ys[i])
...
def _plot_joint_evaluation(
    ...
    agent_xs / agent_ys: list of length num_agents; each entry is a list of
    per-episode trajectories (list[list[float]]).
    """
    print("\nGenerating joint evaluation plots...")

    # Linestyle cycle for distinguishing agents (works for up to 4).
    agent_linestyles = ["--", ":", "-.", (0, (3, 1, 1, 1))]

    fig, axes = plt.subplots(2, 3, figsize=(15, 9))
    fig.suptitle(f"Joint Policy Evaluation  (n={num_episodes} episodes, N={num_agents} agents)",
    ...
    for i in range(min(5, len(patch_xs))):
        ...
        for a in range(num_agents):
            if i < len(agent_xs[a]) and agent_xs[a][i]:
                ls = agent_linestyles[a % len(agent_linestyles)]
                ax.plot(agent_xs[a][i], agent_ys[a][i], color=c, linewidth=1,
    ...
    agent_legend = ", ".join(
        f"{agent_linestyles[a % len(agent_linestyles)]}=A{a}" for a in range(num_agents)
    )
    ax.set_title(f"Trajectories (solid=patch, {agent_legend})")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.axis("equal")
    ax.grid(True, alpha=0.3)
    ...
    ax.fill_between(t, np.clip(m - s, 0, num_agents),
                    np.clip(m + s, 0, num_agents), alpha=0.25, color="blue")
    ax.set_ylim(-0.1, num_agents + 0.5)
    ax.set_title("Agents Inside Patch (mean ± std)")
    ax.set_xlabel("Step")
    ax.set_ylabel("Count")
    ax.axhline(num_agents, color="green", linestyle="--", alpha=0.5, label="All inside")
    ...
    ax = axes[1, 2]
    denom = max(num_agents, 1)
    inside_fracs = [np.mean(ts) / denom for ts in agents_inside_ts]  # 0-1 fraction of max
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("Avg Fraction of Steps All Agents Inside")
    ax.set_xlabel("Episode")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-agents", type=int, default=1, choices=[1, 2, 3, 4],
                       help="Number of learning agents in the joint env (must match agent policy obs dim)")
